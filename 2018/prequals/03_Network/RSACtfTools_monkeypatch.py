            ...
            for y in self.attackobjs:
                if x.pub_key.n != y.pub_key.n:
                    ...
                    if g != 1:
                        ...
                        with open("%s.key" % x.pubkeyfile, "w") as f:
                            f.write(str(x.priv_key))
                        with open("%s.key" % y.pubkeyfile, "w") as f:
                            f.write(str(y.priv_key))
...
if __name__ == "__main__":
    ...
    if args.publickey:
        if "*" in args.publickey:
            pass
        elif not args.n or not args.e:
            key = open(args.publickey, 'rb').read()
            pkey = PublicKey(key)
            if not args.n:
                args.n = pkey.n
